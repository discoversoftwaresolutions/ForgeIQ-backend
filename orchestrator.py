...
# V0.5 – Unified Control Plane (Corrected)
# ==============================================
import os
import json
import asyncio
...
class Orchestrator:
    def __init__(
        ...
        if not forgeiq_sdk_client:
            logger.warning("ForgeIQ SDK client not provided — MCP SDK optimization disabled")
    ...
    async def _update_flow_state(
        ...
        initial_state_if_new: Optional[Dict[str, Any]] = None,
    ):
        ...
        if not state:
            if not initial_state_if_new:
                raise OrchestrationError("Flow state missing", flow_id)
            state = initial_state_if_new.copy()
    ...
    async def request_and_apply_mcp_optimization(
        ...
        try:
            with span:
                self.mcp_metrics.increment("mcp.optimization.requests")
                ...
                self.mcp_memory.record_event(
                    "dag_snapshot",
                    {"flow_id": flow_id, "dag": context.current_dag_snapshot},
                )
                ...
                if response and response.get("status") == "strategy_provided":
                    details = response.get("strategy_details") or {}
                    raw_dag = details.get("new_dag_definition_raw")

                    if raw_dag:
                        optimized_dag = DagDefinition(**raw_dag)

                        await self._update_flow_state(
                            flow_id,
                            {"current_stage": f"MCP_OPTIMIZATION_RECEIVED_{optimized_dag['dag_id']}"},
                        )

                        send_proprietary_audit_event(
                            event_type="MCP_STRATEGY_APPLIED",
                            payload={
                                "flow_id": flow_id,
                                "project_id": project_id,
                        ...
                        self.mcp_metrics.increment("mcp.optimization.success")
                ...
                self.mcp_metrics.increment("mcp.optimization.noop")
                await self._update_flow_state(
                    flow_id,
            ...
            self.mcp_metrics.increment("mcp.optimization.error")

            logger.error("MCP optimization failed", exc_info=True)
            await self._update_flow_state(
                flow_id,
                {
                    "current_stage": "MCP_OPTIMIZATION_ERROR",
                    "error_message": str(e),
                },
            )

            send_proprietary_audit_event(
                event_type="MCP_OPTIMIZATION_FAILURE",
                payload={
                    "flow_id": flow_id,
                    "project_id": project_id,
                    "error": str(e),
